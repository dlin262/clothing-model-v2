...
# ── PINs (set via env vars on Railway) ────────────────────────────────────────
...
def index():
    if session.get('role') == 'staff':
        return redirect(url_for('staff'))
    return render_template('index.html', role='owner')
...
def get_items():
    ...
    p = ph(db)
    if status == 'instock':
        cur.execute('SELECT * FROM items WHERE quantity>0 ORDER BY name')
    ...
    for item in rows:
        ...
        ts, tr, tc = item_extras(cur, db, item['id'])
...
def add_item():
    ...
    cur.execute(
        f'INSERT INTO items (name,quantity,buy_price,sale_price,image_path,category) VALUES ({p},{p},{p},{p},{p},{p})',
...
def update_item(item_id):
    ...
    cur.execute(
        f'UPDATE items SET name={p},quantity={p},buy_price={p},sale_price={p},category={p} WHERE id={p}',
...
def record_sale(item_id):
    ...
    cur.execute(
        f'INSERT INTO sales (item_id,quantity_sold,buy_price,sale_price,note) VALUES ({p},{p},{p},{p},{p})',
...
def get_sales():
    conn, db = get_db(); cur = conn.cursor()
    cur.execute('''
        SELECT s.id, s.item_id, s.quantity_sold, s.buy_price, s.sale_price, s.note,
...
def export_excel():
    ...
    all_sales = fetchall(cur); conn.close()
    ...
    def hdr_row(ws, row, headers, bg=BLACK, fg=ACCENT):
        for i,h in enumerate(headers,1):
            c=ws.cell(row=row,column=i,value=h)
            c.fill=hfill(bg); c.font=Font(bold=True,color=fg,name='Calibri',size=10)
            c.alignment=Alignment(horizontal='center',vertical='center')
        ws.row_dimensions[row].height=18

    def dcell(ws,row,col,val,fmt=None,color=None,alt=False):
        c=ws.cell(row=row,column=col,value=val)
        c.fill=hfill('F0EDE8' if alt else 'FFFFFF'); c.font=Font(name='Calibri',size=10,color=color or BLACK)
    ...
    conn2, db2 = get_db(); cur2 = conn2.cursor()

    def inv_sheet(ws, items, title):
        HDRS=['#','Name','Category','Buy Price (฿)','Sale Price (฿)',
              'Profit/Unit (฿)','Qty','Status','Total Sold','Total Revenue (฿)','Total Profit (฿)']
        write_title(ws,title,len(HDRS)); hdr_row(ws,3,HDRS)
        for ri,item in enumerate(items,1):
            row=3+ri; alt=(ri%2==0)
            ts,tr,tc=item_extras(cur2,db2,item['id'])
            ...
            fmts=[None,None,None,'#,##0.00','#,##0.00','#,##0.00',None,None,None,'#,##0.00','#,##0.00']
        ...
        setw(ws,[4,30,14,14,16,16,7,13,11,18,16]); ws.freeze_panes='A4'
    conn2.close()

    ws1=wb.active; ws1.title='All Items'; inv_sheet(ws1,all_items,'All Inventory Items')
    ws2=wb.create_sheet('In Stock'); inv_sheet(ws2,[i for i in all_items if int(i['quantity'])>0],'In Stock Items')
    ws3=wb.create_sheet('Out of Stock'); inv_sheet(ws3,[i for i in all_items if int(i['quantity'])==0],'Out of Stock Items')

    ws4=wb.create_sheet('Sales History')
    SHDRS=['#','Item','Qty Sold','Buy (฿)','Sale (฿)','Revenue (฿)','Profit (฿)','Note','Date']
    write_title(ws4,'Sales History',len(SHDRS)); hdr_row(ws4,3,SHDRS)
    for ri,s in enumerate(all_sales,1):
        row=3+ri; alt=(ri%2==0)
        rev=float(s['quantity_sold'])*float(s['sale_price'])
        pft=float(s.get('profit') or 0)
        vals=[ri,s['item_name'],s['quantity_sold'],float(s['buy_price']),float(s['sale_price']),rev,pft,s.get('note') or '',str(s['sold_at'])[:16]]
        fmts=[None,None,None,'#,##0.00','#,##0.00','#,##0.00','#,##0.00',None,None]
        for ci,(v,f) in enumerate(zip(vals,fmts),1):
            clr=GREEN if ci==7 and pft>=0 else RED if ci==7 else None
            dcell(ws4,row,ci,v,fmt=f,color=clr,alt=alt)
    setw(ws4,[4,28,10,12,13,14,13,24,18]); ws4.freeze_panes='A4'

    ws5=wb.create_sheet('Summary'); write_title(ws5,'Business Summary',2)
    ...
    summary=[
        ...
        ('Total Revenue (฿)',total_rev,'#,##0.00'),
        ('Total Cost (฿)',total_cost,'#,##0.00'),
        ('Total Profit (฿)',total_pft,'#,##0.00'),
        ('Profit Margin',margin/100,'0.00%'),
        ('','',''),
        ('Inventory Value at Sale Price (฿)',sum(float(i['quantity'])*float(i['sale_price']) for i in all_items),'#,##0.00'),
        ('Inventory Value at Buy Price (฿)', sum(float(i['quantity'])*float(i['buy_price'])  for i in all_items),'#,##0.00'),
    ]
    for ri,(label,value,fmt) in enumerate(summary,4):
        ...
        pcolor=GREEN if label=='Total Profit (฿)' and total_pft>=0 else RED if label=='Total Profit (฿)' else BLACK
